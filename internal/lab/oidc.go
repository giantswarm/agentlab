...
// labHTTPClient returns a client that trusts the lab CA (certs/ca.crt) on top
// of the system pool, so it can talk to both Dex (lab TLS) and plain-HTTP
// services with one client.
func labHTTPClient(timeout time.Duration) (*http.Client, error) {
	caPEM, err := os.ReadFile("certs/ca.crt")
	if err != nil {
		return nil, fmt.Errorf("reading lab CA (run `dexlab up` first?): %w", err)
	}
...
func passwordGrant(cfg *config.Config, clientID, clientSecret, email, password, scope string) (string, error) {
	...
	if err := json.Unmarshal(body, &tok); err != nil || tok.IDToken == "" {
		...
		return "", fmt.Errorf("dex login failed for %s: %s", email, msg)
	}
