...
func newFakePortal(t *testing.T) *fakePortal {
	...
	fp.mux.HandleFunc(portalMusterAPI+"/call", func(w http.ResponseWriter, r *http.Request) {
		...
		if isError {
			writeJSON(w, http.StatusInternalServerError, map[string]any{fieldError: map[string]any{nameKey: "Error", fieldMessage: text}, "response": map[string]any{"statusCode": http.StatusInternalServerError}})
			return
		}
