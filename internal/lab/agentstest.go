...
// agentsTestV2 is the headless proof, on kagent API v2, that agent-manager acts
// as the signed-in user, through the platform path only (Dex id_token -> muster -> call_tool
...
// nothing behind.
func agentsTestV2(cfg *config.Config, email string) error {
	...
	if _, err := session.callServerTool(toolPrefix+"get_agent", map[string]any{nameKey: agentsTestAgent}); err == nil {
		...
		if _, err := session.callServerTool(toolPrefix+"delete_agent", map[string]any{nameKey: agentsTestAgent, forceKey: true}); err != nil {
			return err
		}
	...
	createArgs := map[string]any{
		nameKey: agentsTestAgent, modelConfigKey: modelConfig, displayNameKey: agentsTestDisplayName,
		descriptionKey:   "Throwaway agent of `agentlab agents-test`; deleted by the same run.",
		systemMessageKey: "Reply with exactly the word pong and nothing else.",
	}
	...
	} else if !strings.Contains(err.Error(), toolsetKey) || !strings.Contains(err.Error(), "preset:none") {
	...
	createArgs[toolsetKey] = []string{agentsTestToolset}
	var created struct {
	...
	if viewer == nil {
		...
		text, err := viewerSession.callServerTool(toolPrefix+"create_agent", map[string]any{nameKey: agentsTestAgent + "-viewer", modelConfigKey: modelConfig, toolsetKey: []string{agentsTestToolset}})
		switch {
		case err == nil:
