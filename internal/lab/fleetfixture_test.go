...
func TestCheckToolGroupLabels(t *testing.T) {
	...
	t.Run("a lab-created server with the platform group is refused", func(t *testing.T) {
		// The platform's own servers (agent-manager, model-manager,
		// vm-manager) are the charts' and carry the group from their
		// templates; nothing the lab creates may claim it.
		...
		if _, err := checkToolGroupLabels(append(fixture(), stray), oauth, true); err == nil || !strings.Contains(err.Error(), vmManagerMCPServer) {
			t.Fatalf("want the lab-created labelled server named, got %v", err)
		}
	})
