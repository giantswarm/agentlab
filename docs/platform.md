...
| `networkPolicy.enabled: false`, `kyvernoPolicies.enabled: false` | The chart's own policy objects. No Kyverno in kind, so its policies would render CRs whose API group the cluster does not serve; the Cilium policies would apply (the API is served, next row) and enforce nothing, so the lab does not pretend to have them. |
