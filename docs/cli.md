...
| Command | What it does |
|---|---|
| `open <portal\|agents>` | Open a lab URL in the browser and print it (so it can be copied where the opener finds no browser — SSH, WSL): `portal` is Backstage, `agents` the kagent UI. The target is required; without one, the refusal names both. Refused when the target is disabled in `agentlab.yaml`, when the cluster is not running, or when the target does not answer yet — a plain fact instead of an error page in the browser. While the lab CA is untrusted, `open portal` offers `agentlab trust` first on a terminal and warns off one; `open agents` is plain HTTP on loopback and asks nothing. |
