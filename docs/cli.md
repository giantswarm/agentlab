...
| `backstage-test [email...]` | The headless Backstage sign-in and the muster hop with that user's own forwarded token, including the per-server Sign in challenge, the MCP servers page's grouping and, on kagent API v2 with agents on, the Agent Platform pages — the agents list for every user, a chat turn for the first (default: every user). See [Backstage](backstage.md). |
