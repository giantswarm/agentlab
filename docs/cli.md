...
| `vm-manager-env` | Print the environment `vm-manager serve` needs for this lab — the listen address pods can dial and OAuth against the lab Dex (issuer, CA, the platform client and its audience) — the same content `agentlab platform` writes to `state/vm-manager.env`. See [vm-manager](vm-manager.md). |
...
| `--vm-manager[=false]` | Pin the host vm-manager wiring on or off instead of following whether one answers on this machine. |
| `--vm-manager-port <port>` | The host port vm-manager listens on (default 8100; `state/vm-manager.env` binds it). |
