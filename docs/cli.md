...
| `klaus-gateway-test [email]` | The Swarmgeist proof: klaus-gateway runs on the host (the released image by default, `--gateway-image`; a local build with `--gateway-binary`) against the lab's public gRPC target `grpcs://agentgateway.<domain>:<port>` with the lab CA, the JWT policy at the edge, a bolt store, and its web channel is driven with the user's forwarded Dex id_token: discovery lists the fixture's `AgentTemplate` with its display-name and icon annotations and hides (and refuses, with the reason) one no Harness admits; one streamed turn binds the thread to one `AgentInstance` and is attributed to the person at muster; a `requireApproval` binding pauses the task at input-required and the decision on the web channel resumes it to completed; a stream the client closes has the task cancelled at the controller and the thread goes on; a restart on the bolt store keeps the thread → `AgentInstance` mapping. `--gateway-port` (default 18090), `--run-dir` keeps the store and the gateway's log, `--ready-timeout` bounds the fixture's boot (default 10m), `--model-config` picks the kagent ModelConfig the fixture runs on ([a host model without an Anthropic key](models.md#agent-proofs-without-an-anthropic-key)). A 0.x gateway image is the documented negative. With `platform.klausGateway` on, the meta chart's `klaus-gateway` component follows: the Role scoped to the OBO link Secret and no store volume, two links written through the gateway's store package with the lab's key, the pod deleted and its replacement Ready with the same links within 60 s, one turn through the pod over the in-cluster target. See [The Swarmgeist proof](platform.md#the-swarmgeist-proof-klaus-gateway-on-kagent-api-v2) and [klaus-gateway](klaus-gateway.md). |
